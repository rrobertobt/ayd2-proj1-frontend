  ...
  <Dialog :open="open" @update:open="$emit('update:open', $event)">
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          Busca y selecciona un desarrollador para el paso
      ...
      <form @submit="onSubmit" class="space-y-4">
        <VeeField v-slot="{ errors, setValue }" name="userId">
          <Field :data-invalid="!!errors.length">
            <FieldLabel>Desarrollador</FieldLabel>
...
const formSchema = z.object({
  userId: z.number({ required_error: "Selecciona un desarrollador" }),
});
...
const { mutate: assignDeveloper, asyncStatus } = casesMutations.useAssignDeveloper({
  onComplete: () => {
...
const onSubmit = handleSubmit((values) => {
  assignDeveloper({ caseId: props.caseId, stepId: props.stepId, userId: values.userId });
});
</script>
