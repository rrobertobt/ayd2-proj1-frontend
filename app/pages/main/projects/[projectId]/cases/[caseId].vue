<template>
  <div>
    ...
    <div v-if="casePending" class="flex items-center gap-2 text-muted-foreground py-8">
    ...
    <template v-else-if="caseData">
      ...
      <div class="mb-6">
        <div class="flex items-start justify-between gap-2 flex-wrap">
          <div>
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <Badge v-if="caseData.overdue" class="bg-red-100 text-red-700">Vencido</Badge>
              <Badge
                :class="{
                  'bg-blue-100 text-blue-700': caseData.status === 'OPEN',
                  'bg-yellow-100 text-yellow-700': caseData.status === 'IN_PROGRESS',
                  'bg-green-100 text-green-700': caseData.status === 'COMPLETED',
            ...
            <p class="text-muted-foreground mt-1">{{ caseData.description || "Sin descripción" }}</p>
            <div class="flex flex-wrap gap-4 mt-2 text-sm text-muted-foreground">
              <span>
                <Icon name="lucide:calendar" class="inline mr-1" />
                Vence: {{ new Date(caseData.dueDate).toLocaleDateString("es-GT") }}
              </span>
              <span>
          ...
          <!-- Admin actions -->
          <div
            v-if="session?.role?.code === 'PROJECT_ADMIN' && caseData.status !== 'COMPLETED' && caseData.status !== 'CANCELED'"
        ...
        <div class="w-full bg-secondary rounded-full h-2 mt-4">
          <div
            ...
            :class="caseData.progressPercent === 100 ? 'bg-green-500' : 'bg-primary'"
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" @click="refreshCase" :disabled="casePending">
            <Icon name="lucide:refresh-cw" :class="{ 'animate-spin': casePending }" />
          </Button>
        </div>

        <div v-if="!sortedSteps.length" class="text-muted-foreground text-sm py-4 border rounded-lg text-center">
        ...
        <div v-else class="space-y-3">
          <Card
            ...
            :class="{
              ...
              'border-yellow-200 bg-yellow-50/50': step.status === 'IN_PROGRESS',
              'border-blue-200 bg-blue-50/50': step.status === 'ASSIGNED' || step.status === 'SUBMITTED',
              'border-red-200 bg-red-50/50': step.status === 'REJECTED',
              'opacity-60 hover:opacity-80': step.status === 'PENDING' && step.stepOrder !== 1 && sortedSteps[step.stepOrder - 2]?.status !== 'APPROVED',
            }"
          >
            <CardContent class="py-4">
              <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div class="flex items-center gap-3">
                  <div
                    ...
                    :class="{
                      ...
                      'bg-blue-500 text-white': step.status === 'ASSIGNED' || step.status === 'SUBMITTED',
                  ...
                  <div>
                    <div class="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge
                        ...
                        :class="{
                          ...
                          'bg-blue-100 text-blue-700': step.status === 'ASSIGNED' || step.status === 'SUBMITTED',
                    ...
                    <p v-if="step.assignedEmployeeName" class="text-sm text-muted-foreground">
                      <Icon name="lucide:user" class="inline mr-1" />
                      {{ step.assignedEmployeeName }}
                    </p>
                    <p v-else class="text-sm text-muted-foreground">Sin asignar</p>
                    <p v-if="step.rejectionReason" class="text-xs text-red-600 mt-1">
                ...
                <div v-if="session?.role?.code === 'PROJECT_ADMIN'" class="flex gap-2 shrink-0">
                  <Button
                    v-if="step.status === 'PENDING'"
                    :disabled="sortedSteps[step.stepOrder - 2]?.status !== 'APPROVED' && step.stepOrder !== 1"
                    size="sm"
                    variant="outline"
                  ...
                  <template v-if="step.status === 'ASSIGNED' || step.status === 'IN_PROGRESS' || step.status === 'SUBMITTED'">
                    <Button
                      size="sm"
                      variant="outline"
                  ...
                  <template v-if="step.status === 'APPROVED'">
                    <Icon name="lucide:check-circle-2" class="text-green-500 h-5 w-5" />
                  </template>
                </div>
              </div>
            </CardContent>
...
<script setup lang="ts">
  ...
  const { data: caseData, pending: casePending, refresh: refreshCase } = await useAsyncData(
    `case-${caseId.value}`,
    () => casesApi.getById(caseId.value),
  );

  const sortedSteps = computed(() =>
    [...(caseData.value?.steps ?? [])].sort((a, b) => a.stepOrder - b.stepOrder),
  );
